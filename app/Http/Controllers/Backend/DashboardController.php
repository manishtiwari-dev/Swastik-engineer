...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // today's earning 
        $todayEarning = Order::where('order_status',1)->where('created_at', '>=', $dayStart)->sum('grand_total_amount');
       
        // today's pending earning 
        $todayPendingEarning = Order::where('order_status',5)->where('created_at', '>=', $dayStart)->sum('grand_total_amount');
    
        // this year earning
        $thisYearEarning = Order::where('order_status',1)->where('created_at', '>=', $yearStart)->sum('grand_total_amount');
      

        // total earning
        $totalEarning = Order::where('order_status', 1)->sum('grand_total_amount');
    ...
    private function totalSalesChart($time)
    {
        ...
        $orders = Order::where('order_status', '!=',2)->where('created_at', '>=', Carbon::now()->subDays($timeline))->oldest();;
        ...
        for ($i = $timeline; $i >= 0; $i--) {
            ...
            foreach ($orders->get() as $order) {
                ...
                if (date('Y-m-d', strtotime($i . ' days ago')) == date('Y-m-d', strtotime($order->created_at))) {
                    $totalSalesAmount += $order->grand_total_amount;
                }
            }
        ...
        $totalSalesData->totalEarning = $orders->sum('grand_total_amount');
    ...
    {
        $monthStart = Carbon::now()->startOfMonth();
        $orders = Order::where('order_status', '!=',3)->where('created_at', '>=', $monthStart)->oldest();
        ...
        foreach ($dates as $key => $date) {
            $totalOrdersAmount = 0;
            foreach ($orders->get() as $order) {
                if ($date == date('Y-m-d', strtotime($order->created_at))) {
                    $totalOrdersAmount += $order->grand_total_amount;
                }
            }
        ...
        $thisMonthSaleData->totalEarning = $orders->sum('grand_total_amount');
